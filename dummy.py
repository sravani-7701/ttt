...
def out(d1,d2,d3):
    Tag = "Offshore"  ## Has to be Onshore/Shared/Offshore
    ...
    conv = {'Account': str, 'Branch': str, 'Cost Centre': str, 'Service Line': str, 'Program': str, 'Project': str,
            'Future1': str}
    dfl = pd.read_csv(d3, None,converters=conv)
    dfl1 = []
    type2 = []
    for type_, df in dfl.items():
        print(type_)
    ...
    return c5.to_excel(f'Output/OUTPUT_{Tag}_{month_name}_{year}.xlsx', index=False)
